...
c[0][a]-5*c[1][a]+4*c[2][a]-c[3][a])*f*f+(-c[0][a]+3*c[1][a]-3*c[2][a]+c[3][a])*f*f*f)}))}d.push(c[2])}return d},l=function(){var b=+$(a).offset().left,c=+$(a).offset().top,f=500/+$(a).width();$(a).off("touchmove").on("touchmove",function(a){var g=(+a.originalEvent.touches[0].pageX-b)*e*f;a=(+a.originalEvent.touches[0].pageY-c)*e*f;var h=d.slice(-1)[0]||[-1,-1];h[0]!==g&&h[1]!==a&&d.push([g,a]);return!1});$(a).off("touchend").on("touchend",function(){d.length&&g(2<d.length?k(d):d);d=[]})};$("#page5").on("scrollstop",
l).on("webkitTransitionEnd",l);$("#form").off("submit").on("submit",function(){c?($("#submit-signature").val(a.toDataURL("image/png")),$("#submit-ua").val(navigator.userAgent),$.post("/api/students/"+h.id+"/submit",$(this).serialize(),function(){f.forward()})):alert("You have not signed yet.");return!1})}).register(6,function(){$("#back-button").addClass("donotpresent").off("tap")}).run()});
