...
function(){n.forward();return!1})}).register(1,function(){a("#page1 .interactive-content ul").empty()},function(){a.getJSON("/api/classes").done(function(c){a.each(c.data,function(d,b){a("#page1 .interactive-content ul").append(a("<li/>").attr("id",b.join("")).text(b.join("")))});a("#page1 .interactive-content ul").off("tap").on("tap","li",function(){g=a(this).attr("id");n.forward();return!1})})}).register(2,function(){if(!g)throw"No class name";a("#page2 .interactive-content ul").empty()},function(){a.getJSON("/api/classes/"+
g).done(function(c){a.each(c.data,function(d,b){a("#page2 .interactive-content ul").append(a("<li/>").attr("id","student"+b[0]).text(b[0]+". "+b[1]))});a("#page2 .interactive-content ul").off("tap").on("tap","li",function(){m=a(this).attr("id").slice(7);n.forward();return!1})})}).register(3,function(){if(!g||!m)throw"No class name or index number";},function(){a("#table-class").text(g);a("#table-index").text(m);var c=function(a){return(a=a.trim().toUpperCase().match(/^(?:[STFG][0-9]{0,4})?[0-9]{3}[JZIHGFEDCBAXWUTRQPNMLK]$/))&&
a[0]};a("#submit-nric").off("change").on("change",function(){var d=c(a(this).val());a(this)[d?"removeClass":"addClass"]("invalid");d&&a(this).val(d)});a("#form").off("submit").on("submit",function(){var d=c(a("#submit-nric").val());d?(v=d,console.log(v),a.getJSON("/api/classes/"+g+"/"+m,{nric:v}).done(function(a){p=a.data;"SubmissionOpen"===p.submission.tag?n.forward():"SubmissionNotOpen"===p.submission.tag?alert("Submission is not open yet. Check back later."):alert("You have already submitted before. You cannot submit again.")}).fail(function(){alert("The NRIC is incorrect.")})):
alert("The NRIC is incorrect.");return!1})}).register(4,function(){if(!g||!m)throw"No class name or index number";},function(){a("#table-chinesename").text(p.chinese_name);a.getJSON("/api/subjects").done(function(d){var b=p.subject_combi;console.log(b);a("#table-subjects").text(a.map(b,function(a){console.log(_.find(d.data,function(b){return b.id===a}).name);return _.find(d.data,function(b){return b.id===a}).name}).join(", ")||"\u2014")});a.getJSON("/api/ccas").done(function(d){a.each([1,2,3],function(b,
